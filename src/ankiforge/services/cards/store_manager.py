...
class StoreManager:
    ...
    def get_card_content(note_ligne: list[str], header_dict: dict) -> list[str]:
        ...
        content_only = [
            val for idx, val in enumerate(note_ligne)
    ...
    def handle_txt(self, txt_path: str | Path):
        ...
        print(f"Début insertion dans la base de donnée")

        with db.atomic():
            for row in cards:
                try:
                    # 1. Extraction sécurisée des métadonnées (avec valeurs par défaut si absentes)
                    deck_name = row[header_dict["deck"]] if "deck" in header_dict and header_dict["deck"] < len(
                        row) else "Default Deck"
                    notetype_name = row[header_dict["notetype"]] if "notetype" in header_dict and header_dict[
                        "notetype"] < len(row) else "Basic"
                    guid = row[header_dict["guid"]] if "guid" in header_dict and header_dict["guid"] < len(
                        row) else str(uuid.uuid4())
                    ...
                    deck_obj, _ = DeckModel.get_or_create(
                        name=deck_name,
    ...
    def handle_apkg(self, apkg_path: Path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                raise ValueError(f"Le fichier {apkg_path.name} n'est pas une archive valide.")
            ...
            try:
                ...
                raise RuntimeError(f"Erreur SQL lors de l'extraction : {e}")
            finally:
                conn.close()
