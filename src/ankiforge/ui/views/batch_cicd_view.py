...
class BatchCicdView(QWidget):
    """
    Card Factory View (Batch Processing) - CI/CD Variant.
    ...
    def _build_metrics_row(self) -> None:
        """Builds the top metrics row with 4 cards."""
        metrics_layout = QHBoxLayout()
        metrics_layout.setSpacing(15)

        self.metric_total = MetricCard("TOTAL DOCS", "0", "📄")
        self.metric_queue = MetricCard("IN QUEUE", "0", "⏳")
        self.metric_tokens = MetricCard("EST. TOKENS", "0", "📊")
        self.metric_cost = MetricCard("EST. COST", "$0.00", "💰")

        metrics_layout.addWidget(self.metric_total)
        metrics_layout.addWidget(self.metric_queue)
        metrics_layout.addWidget(self.metric_tokens)
        metrics_layout.addWidget(self.metric_cost)
    ...
    def _build_source_panel(self) -> None:
        ...
        lbl_source.setStyleSheet("font-weight: bold; color: palette(placeholder-text); font-size: 11px; letter-spacing: 1px;")
    ...
    def _build_queue_panel(self) -> None:
        ...
        lbl_config.setStyleSheet("font-weight: bold; color: palette(placeholder-text); font-size: 11px; letter-spacing: 1px;")
        queue_layout.addWidget(lbl_config)
        queue_layout.addSpacing(5)
        ...
        queue_layout.addSpacing(15)
        lbl_queue = QLabel(self.tr("2. QUEUE"))
        lbl_queue.setStyleSheet("font-weight: bold; color: palette(placeholder-text); font-size: 11px; letter-spacing: 1px;")
    ...
    def _build_console_panel(self) -> None:
        ...
        lbl_console.setStyleSheet("font-weight: bold; color: palette(placeholder-text); font-size: 11px; letter-spacing: 1px;")
        console_layout.addWidget(lbl_console)
        console_layout.addSpacing(5)
        ...
        self.lbl_estimate.setStyleSheet("font-weight: bold; color: palette(highlight); font-size: 13px; padding: 10px; border: 1px dashed palette(highlight); border-radius: 6px;")
        ...
        self.lbl_disclaimer = QLabel(
            self.tr(
                ...
                "⚠️ <b>Warning</b> : These values are purely estimates. ankiforge_obsidian declines all responsibility for actual costs billed by API providers.</i>"
            )
        )
        ...
        self.lbl_status.setStyleSheet("color: palette(placeholder-text); font-size: 12px;")
        self.lbl_status.setContentsMargins(10, 0, 10, 0)
        bottom_layout.addWidget(self.progress_bar)
    ...
    def load_tree_source(self) -> None:
        ...
        self.metric_total.set_value(str(total_docs))
    ...
    def add_selected_to_queue(self) -> None:
        ...
        for item in selected_items:
            ...
            if data.get("type") == "doc":
                ...
                folder_docs = DocumentModel.select().where(cast(Any, DocumentModel).folder_id == data["id"])
    ...
    def _check_ready_state(self) -> None:
        ...
        self.metric_queue.set_value(str(count))
    ...
    def _update_estimates(self) -> None:
        ...
        if total_tokens == 0:
            ...
            self.lbl_estimate.setStyleSheet("font-weight: bold; color: palette(placeholder-text); font-size: 13px; padding: 10px; border: 1px dashed palette(alternate-base); border-radius: 6px;")
            self.metric_tokens.set_value("0")
            self.metric_cost.set_value("$0.00")
            ...
            self.lbl_estimate.setStyleSheet("font-weight: bold; color: #FF9800; font-size: 13px; padding: 10px; border: 1px dashed #FF9800; border-radius: 6px;")
            self.metric_tokens.set_value(str(total_tokens))
            self.metric_cost.set_value(f"${total_cost:.3f}")
        else:
            ...
            self.lbl_estimate.setStyleSheet("font-weight: bold; color: #4CAF50; font-size: 13px; padding: 10px; border: 1px dashed #4CAF50; border-radius: 6px;")
            self.metric_tokens.set_value(str(total_tokens))
            self.metric_cost.set_value("Free")
