"""
Dialogue de Prévisualisation et d'Importation de Modèle de Carte JSON.
"""
...
class ModelImportDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        # Corps principal : Splitter gauche (Infos / options) vs droite (Aperçu WebEngine)
        splitter = QSplitter(Qt.Orientation.Horizontal)

        # Gauche : Paramètres & Métadonnées
        left_widget = QWidget()
        ...
        lbl_meta_author = QLabel(f"<b>Auteur :</b> {author}")
        ...
        # Droite : Aperçu WebEngine Live
        right_widget = QWidget()
        right_layout = QVBoxLayout(right_widget)
        right_layout.setContentsMargins(10, 0, 0, 0)
        right_layout.setSpacing(6)

        lbl_preview_title = QLabel("APERÇU EN DIRECT :")
    ...
    def _render_preview(self) -> None:
        ...
        mock_fields = {f: f"Exemple de contenu pour {f}" for f in fields_list}
        if "Front" in mock_fields:
            mock_fields["Front"] = "Qu'est-ce que le modèle AnkiForge ?"
        if "Back" in mock_fields:
            mock_fields["Back"] = "Un gabarit riche avec CSS modulaire et snippets interactifs."
        if "Texte" in mock_fields:
            mock_fields["Texte"] = "La capitale de la France est {{c1::Paris}}."
        if "Extra" in mock_fields:
            mock_fields["Extra"] = "Paris est la ville la plus peuplée de France."
