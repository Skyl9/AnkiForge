...
- **Batch Slice Composer**: single modal `BatchSliceComposerDialog` (document → mode de découpage → parties → insertion) with live updates: step 1 uses two selectable mode cards (Direct / Auto, style page d'accueil) + embedded `AutoSliceWidget`; step 2 = Direct → `DocumentScopeWidget` (1 partie cochée = 1 tâche, branches agrégées via `scope_result["parts"]` with backward-compatible chunk fallback) or Auto → checkbox list of generated slices (`SliceUnit`), chunk→task fallback via `resolve_chunks`, and scope-memory prefill that never overrides live selection
